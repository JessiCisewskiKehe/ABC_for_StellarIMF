...
# Example using observations generated from the Kroupa (2001) IMF model (kroupa_model.txt)
# No observational effects were applied to kroupa_model.txt so the model specifications have
#  been set below to match this
# For a new dataset, the completeness function, age, and measurement error would need to be
#  updated
...
# Currently set so that no completeness is applied
C_low <- -1
C_up <- 0
...
sigma <-  0  #measurement error
age <-  0  #age of cluster in Myr
	

which.tol = c(1,2)  # using both summary statistics
tolSeq <- c(NA,NA)	
thetaTrue <- cbind(k0input, alphaInput, gammaInput) #Unknown
...
Mimf <- scan("kroupa_model.txt")

#Put through forward model (no observational effects set for kroupa_model.txt)
